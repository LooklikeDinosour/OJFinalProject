...
function EnInQurylist({ checkPermission }) {
    ...
    const list = async () => {//현재 목록 불러오기
        ...
        const response = await axios.get(`http://13.209.147.231:8888/api/main/csEngineerList?currentPage=${currentPage}&postsPerPage=${postsPerPage}&cs_writer=${checkPermission.sub}&role=${checkPermission.role}`)
