...
function EnWorkDetail({ checkPermission }) {
    ...
    useEffect(() => {
        axios.get(`http://43.201.243.223:8888/api/main/engineer/workDetail/${eng_enid}`).then((response) => {
    ...
    const submit = async () => {
        try {
            ...
            if (fileList.length !== 0) {
                //작업내역
                await axios.post("http://43.201.243.223:8888/api/main/engineer/workDetail", workInfoVO);

                //작업내역서 첨부파일
                const response = await axios.post(
                    "http://43.201.243.223:8888/api/main/cloudMultiUpload",
                    formData,
                    {
                        headers: {
    ...
    const handleWorkStatusChange = (server_id, e) => {
        ...
        try {
            ...
            console.log(copy)
            // 서버로 상태값을 보냅니다.
            axios.post("http://43.201.243.223:8888/api/main/engineer/updateWorkStatus", copy);
