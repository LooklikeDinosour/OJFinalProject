...
function UserInQuryWrite({ checkPermission }) {
    ...
    const noticeTypeChange=async(e)=>{
        if(e.target.value=="프로젝트 및 서버 관련"){
            ...
            const response=await axios.get(`http://43.201.243.223:8888/api/main/user/list/${cus_id}`)
    ...
    const proChange=async(e)=>{
        if(e.target.value=="ALL"){
            ...
            const response=await axios.get(`http://43.201.243.223:8888/api/main/user/getServerList?pro_id=${pro_id}`)
