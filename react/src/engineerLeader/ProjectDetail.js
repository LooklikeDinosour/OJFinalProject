...
function EnglProjectDetail(props) {
  ...
  useEffect(() => {
    axios.get(`http://43.201.243.223:8888/api/main/engleader/projectDetail/${pro_id}`)
      .then(response => {
    ...
    axios.post('http://43.201.243.223:8888/api/main/alarm/assignEmer', { engid: engid, serverId: serverId })
                  ...
                  {list.map((data, key) => (
                    <tr key={key}>
                      ...
                      <td>
                        {data.eng_enid ? (
                          <Link
                            className="toworkinfobtn"
                            to={{
                              pathname: `/engineerleader/workinfo/${data.server_id}`,
                            }}
                          >
                            <button>내역확인</button>{" "}
                          </Link> // 데이터가 있으면
